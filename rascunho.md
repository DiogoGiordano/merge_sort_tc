...
Descrição do problema: instâncias, entradas, saídas e restrições, com pelo menos um exemplo pequeno.
Justificativa de tratabilidade: por que o problema pertence à classe P. A justificativa deve indicar o que mede o tamanho da entrada e por que o número de passos do algoritmo é limitado por um polinômio nesse tamanho.
Exemplos de aplicação: situações reais em que o problema aparece.
Algoritmo implementado: ideia central, funcionamento passo a passo em um exemplo e relação com a implementação entregue.
Desenvolvimento
...
ex de funcionamento: Nível 0: 8 elementos /
Nível 1: 4 4 / \ /
Nível 2: 2 2 2 2 / \ / \ / \ /
Nível 3: 1 1 1 1 1 1 1 1

como acontece a divisão do array ao meio: 8 > 4 > 2 > 1

Por isso o log₂( 8 ) = 3

e então o n=8 seria a quantidade de elementos processadas a cada nivel Nível 0 = 8 elementos Nível 1 = 8 elementos Nível 2 = 8 elementos Nível 3 = 8 elementos
...
Recursiva: A forma recursiva de implementação espelha uma definição matemática de divisão e conquista, onde ele divide um array pela metade de forma recursiva até chegar em apenas um elemento, e então combina os dois valores (de forma ordenada neste momento)

Iterativa: trata o array como um elemento individual e usa loops iterativamente para combinar pares adjacentes em blocos ordenados maiores

ideia central, funcionamento passo a passo em um exemplo e relação com a implementação entregue.


os requistos dele que são: Problema "marge-sort"
...
O código deve resolver o problema do tema escolhido com o algoritmo indicado.

O repositório deve conter instruções de execução e pelo menos uma instância de exemplo com a saída esperada.

6. Entregáveis e prazo

Apresentação de slides em PDF.


A entrega será feita pelo Moodle da disciplina até 14/09/2026, às 23h59

O que entregar
Slides da apresentação em PDF.
Código em Python em um repositório GitHub compartilhado com o usuário paulosevero, com instruções de execução e pelo menos uma instância de exemplo com a saída esperada.


Apresentação
15 minutos por grupo, com no mínimo: descrição do problema (instâncias, entradas, saídas e restrições), justificativa de que o problema está em P, exemplos de aplicação e explicação do algoritmo implementado. Depois da apresentação farei perguntas ao grupo e a cada integrante.

Avaliação
O Seminário vale 30% da nota final. Metade da nota é do grupo (apresentação e implementação) e metade é individual (participação e domínio do conteúdo). Domínio do conteúdo sozinho vale 40%, então todos os integrantes precisam entender o problema e o código, não só quem apresentar aquela parte.

Uso de IA Generativa
Permitido. O grupo continua responsável por tudo que entregar e apresentar. Recomendo guardar no repositório os arquivos usados na interação com as ferramentas, como CLAUDE.md ou AGENTS.md, porque podem entrar na discussão.
